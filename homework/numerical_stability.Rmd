...
y_new <- mean_new + Sigma_cross %*% Sigma_obs_inv %*% matrix(y_obs - mean_obs, ncol = 1)
...
```{r}
var_new <- Sigma_new - Sigma_cross %*% Sigma_obs_inv %*% t(Sigma_cross)

# calculate confidence intervals for predicted values
ci_lower <- y_new - 1.96 * sqrt(diag(var_new))
ci_upper <- y_new + 1.96 * sqrt(diag(var_new))
...
df <- data.frame(
  x = c(loc_obs, loc_new), 
  y = c(y_obs, y_new), 
  predicted = c(rep(FALSE, n_obs), rep(TRUE, n_new)), 
  lower = c(rep(NA, n_obs), ci_lower), 
  upper = c(rep(NA, n_obs), ci_upper)
)
ggplot(df, aes(x = x, y = y, color = predicted)) + 
  geom_point() + 
  scale_color_manual(values = c("navy", "deepskyblue")) + 
  geom_linerange(aes(ymin = lower, ymax = upper), color = "black", lwd = 0.4) + 
  theme_bw()
```
...
matern_cov <- function(dist, range) { 
  ...
  return(
    (1 + sqrt(5) * scaled_dist + 5 / 3 * scaled_dist^2) * exp(- sqrt(5) * scaled_dist)
  )
}
```
