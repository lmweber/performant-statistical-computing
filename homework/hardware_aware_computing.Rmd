...
# check difference within reasonable precision
ar_precision_matvec(v, auto_corr) - ar_precision_matvec_R(v, auto_corr)

all(ar_precision_matvec(v, auto_corr) - ar_precision_matvec_R(v, auto_corr) < 1e-8)
```
...
out_matmat <- row_col_dot_matmat(A, B)
dim(out_matmat)
out_matmat[1:6, 1:6]
...
# check
out_matmat_col <- col_oriented_matmat(A, B)
dim(out_matmat_col)
out_matmat_col[1:6, 1:6]

sum(abs(out_matmat_col - out_matR))
...
# check
out_eigen <- matmat_via_eigen(A, B)
out_eigen[1:6, 1:6]

sum(abs(out_eigen - out_matR))
